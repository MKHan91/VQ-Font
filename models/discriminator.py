...
class ProjectionDiscriminator(nn.Module):
    ...
    def __init__(self, C, n_fonts, n_chars, w_norm='spectral', activ='none'):
        ...
        self.stru_emb = w_norm(nn.Embedding(13, C))
    ...
    def forward(self, x, font_indice, char_indice,stru_indice):
        ...
        stru_emb = self.stru_emb(stru_indice)

        font_out = torch.einsum('bchw,bc->bhw', x, font_emb).unsqueeze(1) #[b,512,8,8], [b,512] -> b188
        char_out = torch.einsum('bchw,bc->bhw', x, char_emb).unsqueeze(1)
        stru_out = torch.einsum('bchw,bc->bhw', x, stru_emb).unsqueeze(1)

        return [font_out, char_out,stru_out]
